...
use Laminas\Di\Definition\IntrospectionStrategy;
...
class ZendDiCompiler
{
    ...
    /**
     * Shared instances which can be injected if Laminas\Mvc is used.
     *
    ...
    public function addMvcSharedInstances(MvcEvent $mvcEvent)
    {
        ...
        $mvcSharedInstances = [
            'Laminas\Mvc\MvcEvent'                           => $mvcEvent,
            'Laminas\View\Renderer\PhpRenderer'              => $serviceManager->get('Laminas\View\Renderer\PhpRenderer'),
            'Laminas\Mvc\ApplicationInterface'               => $application,
            'Laminas\ServiceManager\ServiceLocatorInterface' => $serviceManager,
            'Laminas\EventManager\EventManagerInterface'     => $application->getEventManager(),
            'Laminas\Router\RouteStackInterface'             => $mvcEvent->getRouter(),
        ];
    ...
    protected function addDefaultSharedInstances()
    {
        ...
        $defaultSharedInstances = [
            'Laminas\Config\Config'       => $this->config, // The merged global configuration
     ...
     * is the component name (Laminas\Db\Adapter\Adapter => Db component). The output
    ...
    protected function writeComponentDependencyInfo(DefinitionList $definitions)
    {
        try {
            ...
            $info .= PHP_EOL;
            $info .= 'Scanned classes are grouped into components (e.g. the Laminas\Mvc\MvcEvent class belongs to the Laminas\Mvc component).' . PHP_EOL .
    ...
    protected function checkInit()
    {
        if (!$this->isInitialized) {
            throw new RuntimeException(
                "ZendDiCompiler must be initialized before instances can be retrieved. If you don't use Laminas\\Mvc, override 'useZendMvc' in the zendDiCompiler config."
            );
        }
    }
